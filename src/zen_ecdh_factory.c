...
ecdh *ecdh_new_curve(lua_State *L, const char *cname) {
	...
	} else if(strcasecmp(curve,"bls381")==0) {
			e = (ecdh*)lua_newuserdata(L, sizeof(ecdh));
			e->seclen = EGS_BLS381;
			e->fieldsize = EFS_BLS381;
			e->rng = NULL;
			e->hash = HASH_TYPE_BLS381;
			e->ECP__KEY_PAIR_GENERATE = ECP_BLS381_KEY_PAIR_GENERATE;
			e->ECP__PUBLIC_KEY_VALIDATE	= ECP_BLS381_PUBLIC_KEY_VALIDATE;
			e->ECP__SVDP_DH = ECP_BLS381_SVDP_DH;
			e->ECP__ECIES_ENCRYPT = ECP_BLS381_ECIES_ENCRYPT;
			e->ECP__ECIES_DECRYPT = ECP_BLS381_ECIES_DECRYPT;
			e->ECP__SP_DSA = ECP_BLS381_SP_DSA;
			e->ECP__VP_DSA = ECP_BLS381_VP_DSA;
